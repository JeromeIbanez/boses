...
# ---------------------------------------------------------------------------
# Phase 3: External benchmark scoring
# ---------------------------------------------------------------------------


def score_benchmark_run(benchmark_run_id: str) -> None:
    """Compare a completed simulation against its benchmark ground truth."""
    from app.models.benchmark import BenchmarkRun, BenchmarkCase
    from app.models.simulation import Simulation
    from app.models.simulation_result import SimulationResult

    db = SessionLocal()
    try:
        run = db.get(BenchmarkRun, benchmark_run_id)
        if not run:
            return
        ...
        case = db.get(BenchmarkCase, run.benchmark_case_id)
        if not case:
            return
        ...
        gt: dict = case.ground_truth or {}
        predicted_dist = agg.sentiment_distribution or {}
        predicted_dominant = _dominant_sentiment(predicted_dist)
        actual_dominant = gt.get("sentiment")

        # Score 1: Sentiment direction (40%)
        direction_match = predicted_dominant == actual_dominant
        direction_score = 1.0 if direction_match else 0.0

        # Score 2: Distribution accuracy (35%)
        if all(k in gt for k in ("positive_pct", "neutral_pct", "negative_pct")):
            gt_dist = {
                "Positive": gt["positive_pct"],
                "Neutral": gt["neutral_pct"],
                "Negative": gt["negative_pct"],
            }
            dist_score = round(1.0 - _jsd(predicted_dist, gt_dist), 3)
        else:
            dist_score = direction_score  # graceful fallback

        # Score 3: Theme overlap (25%)
        gt_themes: list[str] = gt.get("top_themes", [])
        predicted_themes: list[str] = agg.top_themes or []
        theme_score = round(_theme_jaccard(predicted_themes, gt_themes), 3)

        overall = round(0.40 * direction_score + 0.35 * dist_score + 0.25 * theme_score, 3)

        run.sentiment_accuracy_score = direction_score
        run.distribution_accuracy_score = dist_score
        run.theme_overlap_score = theme_score
        run.overall_accuracy_score = overall
        run.score_breakdown = {
            "direction_match": direction_match,
            "direction_score": direction_score,
            "distribution_accuracy_score": dist_score,
            "theme_overlap_score": theme_score,
            "overall_accuracy_score": overall,
            "predicted_sentiment": predicted_dominant,
            "actual_sentiment": actual_dominant,
            "predicted_distribution": predicted_dist,
            "actual_distribution": gt_dist if "positive_pct" in gt else None,
            "predicted_themes": predicted_themes[:5],
            "actual_themes": gt_themes[:5],
        }
        run.status = "complete"
        run.completed_at = datetime.utcnow()
        db.commit()
        logger.info(f"Benchmark run {benchmark_run_id} scored: overall={overall}")

    except Exception as e:
        logger.error(f"Failed to score benchmark run {benchmark_run_id}: {e}")
        try:
            run = db.get(BenchmarkRun, benchmark_run_id)
            if run:
                run.status = "failed"
...
def maybe_score_benchmark(simulation_id: str) -> None:
    """Called after a simulation finishes. Triggers benchmark scoring if linked."""
    ...
    try:
        bench_run = db.execute(
            select(BenchmarkRun).where(BenchmarkRun.simulation_id == simulation_id)
        ).scalar_one_or_none()
        if not bench_run:
            return

        sim = db.get(Simulation, simulation_id)
        if sim and sim.status in ("complete", "failed"):
            run_id = str(bench_run.id)
    except Exception as e:
        logger.error(f"maybe_score_benchmark lookup failed for {simulation_id}: {e}")
    finally:
        db.close()

    if run_id:
        score_benchmark_run(run_id)
