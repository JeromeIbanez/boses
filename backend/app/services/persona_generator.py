...
from app.services.grounding import format_grounding_context, get_grounding_stats, get_grounding_version
...
class SyntheticPersonaSource(PersonaDataSource):
    ...
    def _pass1_skeletons(self, group: PersonaGroup, grounding_context: str, count: int | None = None) -> list[dict]:
        ...
        quota_block = _compute_distribution_quota(n, group.location or "", group.income_level or "") or ""
...
{{
  ...
  "dominant_brand_stance": "one of: Loyal, Skeptical, Aspirational, Burned, Indifferent, Vocal Advocate"
    ...
    def _expand_one_skeleton(
        ...
        # data_source_references are set post-hoc from actual injected sources — model does not self-cite.
...
{{
  ...
  "ocean_profile": {{
    "openness": <float 0.0-1.0, derived from curiosity, creativity, and adaptability shown in the profile>,
    "conscientiousness": <float 0.0-1.0, derived from discipline, organisation, and reliability>,
    "extraversion": <float 0.0-1.0, derived from social energy, assertiveness, and media habits>,
    "agreeableness": <float 0.0-1.0, derived from cooperativeness, trust, and conflict avoidance>,
    "neuroticism": <float 0.0-1.0, derived from anxiety, financial stress, and emotional volatility in pain points>
  }}
}}
...
Be hyper-specific. Generic statements are not allowed."""
        ...
        # Authoritative sources only — never accept LLM self-citations.
        raw["data_source_references"] = grounding_sources if grounding_sources else []

        # Carry dominant_brand_stance from skeleton (not re-generated by Pass 2).
        raw["dominant_brand_stance"] = skeleton.get("dominant_brand_stance", "")
...
def _compute_distribution_quota(n: int, location: str, income_level: str) -> str | None:
    """
    Return a quota instruction block for Pass 1 when no specific income filter is set.
    Distributes n personas proportionally across income bands from grounding data.
    Returns None if income is filtered or no grounding data is available.
    """
    if income_level and income_level.lower() not in ("any", "all", ""):
        return None
    stats = get_grounding_stats(location)
    if not stats:
        return None
    inc = stats.get("income_distribution", {})
    bands = [(k, v) for k, v in inc.items() if k != "description" and isinstance(v, float)]
    if not bands:
        return None

    total_pct = sum(v for _, v in bands) or 1.0
    quotas = []
    assigned = 0
    for i, (band, pct) in enumerate(bands):
        if i == len(bands) - 1:
            count = n - assigned
        else:
            count = max(0, round(n * pct / total_pct))
            assigned += count
        if count > 0:
            quotas.append(f"  - {band.replace('_', '-').title()}: {count} persona(s)")

    lines = [
        "\nDISTRIBUTION QUOTA — assign income_level to each archetype to match real population proportions:",
        *quotas,
        "(Derived from World Bank / national statistics for this market — treat as hard constraints.)\n",
    ]
    return "\n".join(lines)
...
def generate_personas(group_id: str) -> None:
    ...
    try:
        ...
        # Initialised here so generation_metadata can reference them regardless of path taken.
        grounding_sources: list[str] = []
        reddit_context: str = ""
        cultural_ctx: str | None = None
        ...
        if remaining > 0:
            if source_class is SyntheticPersonaSource:
                # Load grounding once (mirrors SyntheticPersonaSource.fetch — all three layers)
                grounding_context, grounding_sources = format_grounding_context(group.location or "")
                ...
                for skeleton in skeletons:
                    ...
                    persona = Persona(
                        ...
                        dominant_brand_stance=_trunc(profile.get("dominant_brand_stance"), 50),
                        ...
                        ocean_profile=profile.get("ocean_profile"),
        ...
        group.generation_metadata = {
            "model": settings.OPENAI_MODEL,
            "prompt_version": "v3.0",
            "grounding_data_version": get_grounding_version(),
            "grounding_sources": grounding_sources if source_class is SyntheticPersonaSource else [],
            "reddit_signals_used": bool(reddit_context) if source_class is SyntheticPersonaSource else False,
            "reddit_caveat": (
                "Reddit signals skew urban, tech-literate, English-speaking — "
                "used for language texture only, not statistical representation"
            ) if source_class is SyntheticPersonaSource else None,
            "ethnography_used": bool(cultural_ctx) if source_class is SyntheticPersonaSource else False,
            "generated_at": _dt.utcnow().isoformat(),
        }
        _flag_modified(group, "generation_metadata")
        db.commit()
